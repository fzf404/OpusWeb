$(function () {
  $('#new').on('click',()=>{
    getActive("/api/active?datacode=0");
  });
  $('#supernew').on('click',()=>{
    getActive("/api/active?datacode=1");
  });
  $('#technologynew').on('click',()=>{
    getActive("/api/active?datacode=2");
  });
  $('#schoolnew').on('click',()=>{
    getActive("/api/active?datacode=3");
  });
  $('#artnew').on('click',()=>{
    getActive("/api/active?datacode=4");
  });
  $('#nonew').on('click',()=>{
    getActive("/api/active?datacode=10");
  });
  $('#testnew').on('click',()=>{
    getActive("/api/active?datacode=404");
